    ...
    .then(d => {
      if (d.data) {
        ...
        if (content) {
          ...
          content.innerHTML = '<div style="display: grid; gap: 12px; font-size: 14px;"><div><strong>Booking ID:</strong><br>' + b.id.substring(0, 8) + '</div><div><strong>Status:</strong><br><span style="padding: 2px 6px; border-radius: 3px; background: ' + (b.status === 'completed' ? '#10b981' : b.status === 'in-process' ? '#3b82f6' : b.status === 'pending' ? '#f59e0b' : '#ef4444') + '; color: white; font-size: 12px;">' + (b.status || 'pending').toUpperCase() + '</span></div><div><strong>Booking Type:</strong><br>' + bookingTypeDisplay + '</div><div><strong>📝 Notes:</strong><br>' + notesDisplay + '</div></div>';
